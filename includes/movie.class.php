...
class Movie{
    ...
    public function deleteEmbed($movie_id, $embed_id){
        
        if ($embed_id==0){
			$e = Movies::find($movie_id);
			if($e){
				$e->embed = '';
				$e->save();
			}
        } else {
			$e = Movie_embed::where('movie_id', $movie_id)->where('id', $embed_id)->first();
			if($e){
				$e->delete();
			}
        }
    ...
    public function deleteAllEmbeds($movie_id){
		$e = Movie_embed::where('movie_id', $movie_id);
		if($e){
			$e->delete();
		}
    }
	...
	public function getLinks($status = null, $lang=false, $p=null, $l=null, $sortby=null){
        ...
        if (!$sortby || $sortby == 'status'){
            ...
            $order = "ORDER BY submitted_links.date_submitted DESC";
        }
        ...
        $e = mysql_query("SELECT submitted_links.*, users.username, movies.title as movie_title, movies.id as movie_id 
                            ...
                            $order $limit") or die(mysql_error());
        if (mysql_num_rows($e)){
            while($s = mysql_fetch_assoc($e)){
                $links[$s['id']] = $s;
